...
def _status_payload():
    return {"status": "Kirian Brain is running"}
...
def test_built_client_and_status_api_are_served(tmp_path: Path):
    ...
    live2d.mkdir()
    (dist / "index.html").write_text("<html>kirian web</html>", encoding="utf-8")
    (assets / "app.js").write_text("console.log('kirian')", encoding="utf-8")
    ...
    assert "kirian web" in root.text
